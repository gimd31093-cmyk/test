...
const Quiz = {
    ...
    submit(e) {
        ...
        if (data.correct === text) {
            ...
            if (data?.include) {
                if (data.every(v => text.include(v))) $(`.btn_box-${idx}`).html(`<div class="btn">정답!</div>`);
                return;
            }
    ...
    start() {
        $(`.middle`).html(Quiz.data.map(v => {
            return `
                <div class="item item-${v.idx}" data-idx="${v.idx}">
                    <h2 class="title">${(Quiz.now + 1).padStart(2, "0")}. ${nowData.slove}</h2>
